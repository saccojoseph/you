...
**YOU is a personal memory you can inspect and take with you.** It keeps track of the people, plans, preferences, and commitments that shape your life, then gives any assistant you authorize just enough context to help.

Tagline: **The last agent you’ll ever need.** Use it as an ambition, supported by the explanation that the user keeps one context layer while changing models and harnesses over time.

The first public release is an interactive prototype. It uses fictional data, stores edits in the browser, and has no live accounts or AI provider connection. Lead with that clarity so people can evaluate the idea without mistaking the demo for a secure finished service.

## GitHub description

> An open source personal context layer for people, plans, preferences, and life. Model neutral, evidence first, user controlled. Interactive prototype.

Suggested topics: `personal-ai`, `mcp`, `knowledge-graph`, `privacy`, `nextjs`, `typescript`, `open-source`.

## Launch post

What if your AI assistant could remember the people and details that matter, without locking that memory inside one model?

I’m building **YOU**, an open source personal context layer. The idea is a memory you own: people, relationships, plans, preferences, open commitments, and the small things that help an assistant be useful. Each memory has a source and a confidence level. You can inspect it, correct it, exclude it, or export it.

The first interactive prototype is now available. It opens with a first-run flow for choosing potential sources, a preferred assistant, a personal engram, and possible routines. It also shows a memory browser, gentle suggestions, people profiles, and a proposed path for Claude, OpenAI, Codex, or any MCP client to access only what you permit. It runs on fictional data, with no live integrations yet.
...
- Open the first engram questionnaire and add one preference.
- Show an inferred memory and its evidence. Confirm it or mark it for review.
- Turn off agent availability for that memory.
- Ask why YOU thinks Mike likes golf; point out the uncertainty.
- Show the connection plan and explain that the prototype does not connect to real accounts or buy tickets.
